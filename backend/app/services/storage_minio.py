...
async def put_avatar(user_id: int, content: bytes, ext: str) -> str:
    await ensure_bucket()
    # удалить старые с любым расширением
    ...
    _client.put_object(_bucket, objname, io.BytesIO(content), length=len(content), content_type=mimetypes.types_map.get(ext, "image/jpeg"))
    return name
