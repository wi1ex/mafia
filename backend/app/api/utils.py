...
def build_app_settings_snapshot_defaults(core_settings_obj: Any, *, default_starts: Sequence[int], snapshot_cls: Any) -> Any:
    ...
    return snapshot_cls(
        ...
        rating_enabled=getattr(core_settings_obj, "RATING_ENABLED", True),
        ...
        rtc_echo_cancellation_enabled=getattr(core_settings_obj, "RTC_ECHO_CANCELLATION_ENABLED", True),
        rtc_noise_suppression_enabled=getattr(core_settings_obj, "RTC_NOISE_SUPPRESSION_ENABLED", True),
        rtc_auto_gain_control_enabled=getattr(core_settings_obj, "RTC_AUTO_GAIN_CONTROL_ENABLED", True),
        ...
        spectators_limit=max(0, int(getattr(core_settings_obj, "SPECTATORS_LIMIT", 10))),
...
def build_app_settings_snapshot_from_row(row: Any, *, default_starts: Sequence[int], snapshot_cls: Any) -> Any:
    ...
    return snapshot_cls(
        ...
        rating_enabled=bool(getattr(row, "rating_enabled", True)),
        ...
        rtc_echo_cancellation_enabled=bool(getattr(row, "rtc_echo_cancellation_enabled", True)),
        rtc_noise_suppression_enabled=bool(getattr(row, "rtc_noise_suppression_enabled", True)),
        rtc_auto_gain_control_enabled=bool(getattr(row, "rtc_auto_gain_control_enabled", True)),
        ...
        spectators_limit=max(0, int(getattr(row, "spectators_limit", 10))),
    ...
    for game_id, head_id, mode_raw, result_raw, black_alive_raw, started_at, finished_at, roles_raw, points_raw, mmr_raw, actions_raw in raw_games:
        ...
        if uid_key is not None and isinstance(roles_raw, dict):
            ...
            if game_mode == "rating":
                ...
                player_points_value = normalize_game_points_value(points_map.get(uid_key, 0))
...
def serialize_game_for_redis(game_dict: Dict[str, Any]) -> Dict[str, str]:
    ...
    return {
        ...
        "first_shot_check": "1" if raw_bool(game_dict.get("first_shot_check"), True) else "0",
...
def game_from_redis_to_model(raw_game: Dict[str, Any]) -> GameParams:
    ...
    return GameParams(
        ...
        first_shot_check=raw_bool(raw_game.get("first_shot_check"), True),
...
def site_settings_out(row) -> SiteSettingsOut:
    ...
    return SiteSettingsOut(
        ...
        spectators_limit=max(0, int(row.spectators_limit)),
...
def public_settings_out(settings) -> "PublicSettingsOut":
    ...
    return PublicSettingsOut(
        ...
        rating_enabled=bool(getattr(settings, "rating_enabled", True)),
        ...
        rtc_echo_cancellation_enabled=bool(getattr(settings, "rtc_echo_cancellation_enabled", True)),
        rtc_noise_suppression_enabled=bool(getattr(settings, "rtc_noise_suppression_enabled", True)),
        rtc_auto_gain_control_enabled=bool(getattr(settings, "rtc_auto_gain_control_enabled", True)),
...
def build_avatar_reset_notice(user_id: int) -> Notif:
    return Notif(
        ...
        title="Аватар",
        text=(
            "Ваш аватар был сброшен администрацией платформы. Пункт правил: 3.3"
        ),
    )
...
def build_nickname_reset_notice(user_id: int, new_username: str) -> Notif:
    ...
    return Notif(
        ...
        title="Никнейм",
        text=(
            "Ваш никнейм был сброшен администрацией платформы. Пункт правил: 3.3. "
        ...
        if isinstance(game_raw, dict) and game_raw:
            try:
                ...
                game_payload = {
                    ...
                    "first_shot_check": model.first_shot_check,
...
async def _fetch_admin_user_game_data(
    session: AsyncSession,
    user_ids: list[int],
) -> tuple[
    ...
    last_games = {user_id: (None, None) for user_id in user_ids}
    games_played = {user_id: 0 for user_id in user_ids}
    games_hosted = {user_id: 0 for user_id in user_ids}
    if not user_ids:
        return last_games, games_played, games_hosted

    user_id_set = set(user_ids)
    game_query = select(Game.id, Game.finished_at, Game.head_id, Game.result, Game.roles)
    if len(user_ids) <= 200:
        game_query = game_query.where(or_(
            Game.head_id.in_(user_ids),
            *(Game.roles.has_key(str(user_id)) for user_id in user_ids),
        ))

    game_rows = (await session.execute(game_query)).all()
    for game_id, finished_at, head_id, result, roles in game_rows:
        try:
            normalized_game_id = int(game_id)
        except (TypeError, ValueError):
            continue

        if str(result or "") in {"red", "black"}:
            try:
                head_user_id = int(head_id)
            except (TypeError, ValueError):
                head_user_id = 0
            if head_user_id in games_hosted:
                games_hosted[head_user_id] += 1
        ...
        for raw_user_id in roles:
            ...
            previous_game_id, previous_finished_at = last_games[user_id]
            if (
                previous_game_id is None
                or previous_finished_at is None
                or (finished_at, normalized_game_id) > (previous_finished_at, previous_game_id)
            ):
                last_games[user_id] = (normalized_game_id, finished_at)
            if str(result or "") in {"red", "black"}:
