    ...
    if user:
        ...
        return AuthOut(
            ...
            user=UserOut(
                ...
                name=user.name, photo_url=user.photo_url, role=user.role
            )
        )

    # новый пользователь → 202 + сессия
    sess = secrets.token_urlsafe(24)
    r: redis.Redis = build_redis()
    await r.setex(
        f"tg_auth:{sess}", SESSION_TTL,
        json.dumps({
            "id": payload.id,
            "username": payload.username,
            "photo_url": str(payload.photo_url) if payload.photo_url else None,
            "suggested_name": " ".join([x for x in [payload.first_name, payload.last_name] if x]).strip()
                              or payload.username or f"id{payload.id}",
        }),
    )
    pending = AuthPending(session=sess, suggested_username=payload.username or None, suggested_name=data.get("first_name"))
