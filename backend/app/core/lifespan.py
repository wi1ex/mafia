...
async def lifespan(app) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            ...
            await conn.execute(text("ALTER TABLE settings ADD COLUMN IF NOT EXISTS rtc_auto_gain_control_enabled BOOLEAN NOT NULL DEFAULT TRUE"))
            await conn.execute(text("ALTER TABLE settings DROP COLUMN IF EXISTS chat_messages_enabled"))
