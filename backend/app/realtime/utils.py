...
async def emit_user_game_participation_changed(user_id: int, *, in_active_game_as_player: bool, room_id: int | None = None) -> None:
    payload = {
        "user_id": int(user_id),
        "in_active_game_as_player": bool(in_active_game_as_player),
        "in_active_game_as_alive_player": bool(in_active_game_as_player),
...
async def clear_users_in_active_alive_game(user_ids: Iterable[int | str], *, room_id: int | None = None, redis_client=None, emit: bool = True) -> None:
    ids = _normalize_active_alive_user_ids(user_ids)
    if not ids:
        return
    ...
    if room_id_int > 0:
        async with r.pipeline() as p:
            ...
            raw_current_rooms = await p.execute()

        ids_to_clear: list[int] = []
        for uid, raw_room_id in zip(ids, raw_current_rooms):
            try:
                current_room_id = int(raw_room_id or 0)
            except Exception:
                current_room_id = 0
            if current_room_id == room_id_int:
                ids_to_clear.append(uid)
...
async def mark_users_in_active_game(user_ids: Iterable[int | str], room_id: int, *, redis_client=None, emit: bool = True) -> None:
    ids = _normalize_active_alive_user_ids(user_ids)
    room_id_int = int(room_id)
    if not ids or room_id_int <= 0:
        return
    ...
    async with r.pipeline() as p:
        for uid in ids:
            await p.sadd(active_game_rooms_key(uid), str(room_id_int))
        await p.execute()
    ...
    for uid in ids:
        await emit_user_game_participation_changed(uid, in_active_game_as_player=True, room_id=room_id_int)


async def clear_users_in_active_game(user_ids: Iterable[int | str], *, room_id: int | None = None, redis_client=None, emit: bool = True) -> None:
    ids = _normalize_active_alive_user_ids(user_ids)
    if not ids:
        return
    ...
    async with r.pipeline() as p:
        for uid in ids:
            if room_id_int > 0:
                await p.srem(active_game_rooms_key(uid), str(room_id_int))
            else:
                await p.delete(active_game_rooms_key(uid))
        await p.execute()
    ...
    for uid in ids:
        active_room_ids = await get_active_game_rooms(uid, redis_client=r)
        next_room_id = min(active_room_ids) if active_room_ids else None
        await emit_user_game_participation_changed(uid, in_active_game_as_player=bool(active_room_ids), room_id=next_room_id)
...
async def sync_user_active_alive_game_marker(r, rid: int, user_id: int, *, emit: bool = True) -> bool:
    ...
    is_alive = False
    if phase != "idle" and not game_finished:
        try:
...
async def persist_join_user_info(r, rid: int, uid: int, username: Optional[str], avatar_name: Optional[str], theme_color: Optional[str] = None, theme_icon: Optional[str] = None) -> None:
    mp: Dict[str, str] = {}
    ...
    if isinstance(theme_icon, str) and theme_icon.strip():
        mp["theme_icon"] = theme_icon.strip()
    if mp:
        try:
...
async def _load_room_creator_roles(r, creators_by_rid: Mapping[int, int]) -> dict[int, str]:
    room_ids = [rid for rid in (_as_int(raw) for raw in creators_by_rid.keys()) if rid > 0]
    if not room_ids:
        return {}

    roles_by_rid: dict[int, str] = {}
    rows: list[Any] = []
    try:
        async with r.pipeline() as p:
            for rid in room_ids:
                await p.hget(f"room:{rid}:params", "creator_role")
            rows = await p.execute()
    except Exception:
        rows = []

    for rid, creator_role in zip(room_ids, rows):
        roles_by_rid[rid] = normalize_user_role(creator_role)

    return roles_by_rid


async def _admin_created_hidden_room_ids(r, items: Iterable[Mapping[str, Any]]) -> set[int]:
    creators_by_rid: dict[int, int] = {}
    for item in items:
        if str(item.get("anonymity") or "visible") != "hidden":
            continue
        rid = _as_int(item.get("id"))
        if rid > 0:
            creators_by_rid[rid] = _as_int(item.get("creator"))

    roles_by_rid = await _load_room_creator_roles(r, creators_by_rid)
    return {rid for rid, role in roles_by_rid.items() if role == ROLE_ADMIN}


async def _load_room_allow_membership(r, room_ids: Iterable[int], viewer_uid: int) -> dict[int, bool]:
    rids = [rid for rid in (_as_int(raw) for raw in room_ids) if rid > 0]
    if not rids or viewer_uid <= 0:
        return {}
    ...
    try:
        async with r.pipeline() as p:
            ...
            raw = await p.execute()
        for rid, allowed in zip(rids, raw):
...
async def filter_rooms_for_viewer(r, items: Iterable[Mapping[str, Any]], role: str | None, uid: int | None) -> List[dict]:
    ...
    if viewer_role == ROLE_MODER:
        ...
        allow_by_rid = await _load_room_allow_membership(r, admin_hidden_rids, viewer_uid)
        out: List[dict] = []
        for item in prepared:
            rid = _as_int(item.get("id"))
            if rid not in admin_hidden_rids:
                out.append(item)
                continue
            if viewer_uid > 0 and (_as_int(item.get("creator")) == viewer_uid or allow_by_rid.get(rid)):
                out.append(item)
        return out
    ...
    allow_by_rid = await _load_room_allow_membership(r, hidden_rids, viewer_uid)
...
async def _hidden_room_viewers(r, rid: int, *, payload: Mapping[str, Any] | None = None) -> tuple[bool, set[int], bool]:
    room_id = _as_int(rid)
    hidden_raw = ""
    creator_hint = 0
    ...
    roles_by_rid = await _load_room_creator_roles(r, {room_id: creator_hint})
    admin_created = roles_by_rid.get(room_id) == ROLE_ADMIN
...
async def emit_rooms_remove_safe(r, rid: int, *, hidden: bool | None = None, viewer_ids: Iterable[int] | None = None, admin_created: bool | None = None) -> None:
    room_id = _as_int(rid)
    payload = {"id": room_id}
    is_hidden = hidden
    target_ids = _normalize_user_ids(viewer_ids)
    admin_created_hidden = bool(admin_created)
    if is_hidden is None:
        is_hidden, auto_ids, admin_created_hidden = await _hidden_room_viewers(r, room_id)
        target_ids.update(auto_ids)
    elif is_hidden and not target_ids:
        _, auto_ids, admin_created_hidden = await _hidden_room_viewers(r, room_id)
        target_ids.update(auto_ids)
    elif is_hidden and admin_created is None:
...
async def process_player_death(r, rid: int, user_id: int, *, head_uid: int | None = None, actor_role: str = "head", phase_override: str | None = None, reason: str | None = None, defer_finish_check: bool = False, ppk: bool = False) -> bool:
    ...
    if removed:
        ...
        if not defer_finish_check:
            try:
                await maybe_finish_game_after_death(r, rid, head_uid=head_uid, dead_user_id=user_id, death_reason=reason)
            except Exception:
...
async def finish_game(r, rid: int, *, result: str, head_uid: int | None = None, reason: str = "auto_finish") -> bool:
    ...
    if player_ids:
        try:
            await clear_users_in_active_alive_game(player_ids, room_id=rid)
        except Exception:
            log.exception("game_finish.active_alive_clear_failed", rid=rid)
        try:
            await clear_users_in_active_game(player_ids, room_id=rid)
        except Exception:
...
async def perform_game_end(ctx, sess: Optional[dict[str, Any]], *, confirm: bool, allow_non_head: bool = False, reason: str = "manual") -> dict[str, Any]:
    ...
    if players_list:
        try:
            await clear_users_in_active_alive_game(players_list, room_id=rid)
        except Exception:
            log.exception("sio.game_end.active_alive_clear_failed", rid=rid)
        try:
            await clear_users_in_active_game(players_list, room_id=rid)
        except Exception:
            log.exception("sio.game_end.active_game_clear_failed", rid=rid)
...
async def gc_empty_room(rid: int, *, expected_seq: int | None = None) -> bool:
    ...
    try:
        ...
        room_admin_created = False
        hidden_remove_targets: set[int] = set()
        if room_anonymity == "hidden":
            room_creator_roles = await _load_room_creator_roles(r, {rid: room_creator})
            room_admin_created = room_creator_roles.get(rid) == ROLE_ADMIN
            if room_creator > 0:
        ...
        try:
            await emit_rooms_remove_safe(
                r,
                rid,
                hidden=(room_anonymity == "hidden"),
                viewer_ids=hidden_remove_targets if room_anonymity == "hidden" else None,
                admin_created=room_admin_created if room_anonymity == "hidden" else None,
            )
        except Exception as e:
