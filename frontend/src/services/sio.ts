...
export function createPublicSocket(namespace: string, opts?: IoOpts): Socket {
  const tok = getAccessToken()
  const s = io(namespace, { ...opts, auth: tok ? { token: tok } : undefined })
  return tok ? wireAuthedSocket(s) : s
}
...
export function startAuthSocket(opts?: { onForceLogout?: () => void }): Socket {
  ...
  authSocket.on('friends_closeness_update', (p:any) => {
    window.dispatchEvent(new CustomEvent('auth-friends_closeness_update', { detail: p }))
  })
  ...
  authSocket.on('user_game_participation_changed', (p:any) => {
    window.dispatchEvent(new CustomEvent('auth-user_game_participation_changed', { detail: p }))
  })

  authSocket.on('profile_sync', (p:any) => {
    window.dispatchEvent(new CustomEvent('auth-profile_sync', { detail: p }))
  })
