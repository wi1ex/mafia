<template>
  <div class="container">
    <div class="card" v-if="token && wsUrl">
      <LiveKitRoom :server-url="wsUrl" :token="token" :connect="true">
        <VideoConference />
        <RoomAudioRenderer />
      </LiveKitRoom>
    </div>
...
import { LiveKitRoom, VideoConference, RoomAudioRenderer } from '@livekit/components-vue'
...
onMounted(async () => {
  const { ws_url, token: tk } = await rtc.requestJoin(rid)
  wsUrl.value = ws_url
  token.value = tk
})
</script>
