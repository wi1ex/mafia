    ...
    <div class="rules-layout">
      <div class="rules-content">
        ...
        <section id="scoring" class="scoring-intro">
          ...
          <h2>6. Скоринг</h2>
          <p>Баллы рассчитываются только в рейтинговых играх. Красная команда — мирные и шериф, чёрная — мафия и дон. За победу каждый игрок команды получает 1 базовый балл, при поражении или ничьей — 0.</p>
          <p>Все дополнительные начисления и штрафы суммируются, затем сумма ограничивается установленными минимумом и максимумом и прибавляется к базовому баллу. Лимиты не применяются к базовому баллу. Итог округляется до сотых и отображается с двумя знаками после запятой.</p>
          <p>Ниже показаны текущие ставки администратора. Для каждой партии ставки, подписи и лимиты сохраняются при старте: последующие изменения настроек не влияют на уже начавшиеся игры. Расчёт выполняется к завершению игры; при исправлении результата баллы пересчитываются по сохранённым правилам этой партии.</p>
...
const nominationConditions = 'В списке кандидатов перед первым голосованием дня только один чёрный, и он реально уходит по голосованию в этот день, включая переголосование или подъём. На момент первого голосования живы больше четырёх игроков, есть живые чёрные и их число не меньше числа живых красных минус два: учитывается угроза победы после ухода красного и следующего отстрела. После выставляющего в порядке речей нет живых красных. Ни один уход по голосованию в этот день не должен завершиться победой чёрных.'
...
const SCORING_SECTIONS: ScoringSection[] = [
  ...
  },
  {
    ...
    rules: [
      ...
      rule('night_shoot_miss_terminal', 'Промах при гарантированной победе', 'Если успешный отстрел принёс бы чёрным победу, штраф получает единственный живой чёрный при промахе либо один из трёх, не совпавший с выстрелами двух остальных. Отсутствие выстрела тоже учитывается. Заменяет обычный штраф за промах. При двух живых чёрных не применяется; итоговое поражение чёрных не требуется.'),
  ...
  },
  {
    ...
    description: 'Правила слома требуют отметки ведущего. Администратор может исправить или снять любую из трёх отметок в истории рейтинговой игры. Используется последнее сохранённое значение; после исправления баллы пересчитываются по сохранённым ставкам игры, кеш статистики участников сбрасывается. Отметка не отменяет проверку остальных условий.',
    rules: [
      rule('vote_black_unchecked_nine_ten', 'Уход чёрного без чёрной проверки при 9–10 живых', 'Со второго дня: штраф самому чёрному, единолично покинувшему игру через голосование, включая переголосование, если на момент итогов этого голосования живы 9 или 10 игроков. Общий подъём, ночной уход и удаления не учитываются. На момент ухода ни в одной активной версии ведущего не должно быть чёрной проверки этого игрока. Удалённая до ухода проверка не защищает от штрафа; последующие изменения версий не меняют оценку. Личные проверки и общая очевидность цвета здесь не проверяются.'),
      ...
      rule('vote_sheriff_nine_red', 'Штраф за вывод шерифа при 9–10 живых', 'Со второго дня: красному, голосовавшему в настоящего шерифа, который фактически ушёл единолично по голосованию, если на момент подведения итогов результирующего голосования было девять или десять живых. Переголосование учитывается, общий подъём — нет. Не голосовавшие в шерифа штраф не получают.'),
      rule('vote_sheriff_nine_black', 'Бонус за вывод шерифа при 9–10 живых', 'При тех же условиях 9–10 живых и единоличного ухода шерифа со второго дня: каждому чёрному, голосовавшему в него. Это дополнительный бонус, который складывается с обычными баллами за голос в противоположную команду, а не заменяет их. Применяется общий лимит суммы допбаллов.'),
      ...
      rule('vote_break_red_to_red', 'Слом в красного на поражение', 'Отмеченный ведущим красный голосовал в другого красного, единолично ушедшего по голосованию в первый день. Сам отмеченный ушёл на предрешённое поражение красных через голосование или подъём во второй день либо по фолам, техфолам или самоубийству в первый/второй день. Учитывается вся группа подъёма и потенциальный следующий отстрел. Также применяется, если второй день не наступил из-за победы чёрных.'),
      rule('vote_break_red_to_red_safe', 'Слом в красного без ухода на поражение', 'При той же отметке ведущего, голосе красного в другого красного и единоличном уходе цели в первый день: применяется вместо усиленного штрафа, если отмеченный не ушёл на предрешённое поражение и чёрные не победили до второго дня. Если он остался, ушёл не на поражение или поражение вызвал уход другого игрока во второй день, применяется этот штраф.'),
      rule('vote_break_red_to_sheriff_extra', 'Дополнительный штраф за слом в шерифа', 'Если при подтверждённом сломе красного в красного выведенная в первый день цель — настоящий шериф, этот штраф добавляется к одному из двух штрафов выше. Отдельная отметка ведущего не нужна. Обычные штрафы за удаления также сохраняются; вся сумма ограничивается общими лимитами.'),
