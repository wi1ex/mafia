                ...
                <tr v-for="row in sanctions" :key="row.id">
                  ...
                  <td class="rule-cell">
                    <select :value="row.reason || ''" :disabled="isSanctionReasonChanging(row)" :aria-label="`Пункт правил для санкции ${row.id}`" @change="updateSanctionReason(row, $event)">
                      ...
                      <option v-for="option in sanctionReasons" :key="option.value" :value="option.value">{{ option.label }}</option>
                    </select>
                  </td>
...
type GameScoringSettings = {
  ...
  vote_black_unchecked_nine_ten: number
  ...
  farewell_black_named_red: number
  farewell_claimant_black_named_red: number
  ...
  vote_black_unchecked_nine_ten_label: string
  ...
  farewell_black_named_red_label: string
  farewell_claimant_black_named_red_label: string
}
...
const scoring = reactive<GameScoringSettings>({
  ...
  vote_black_unchecked_nine_ten: -0.1,
  ...
  farewell_black_named_red: 0.1,
  farewell_claimant_black_named_red: 0.15,
  fourth_foul_label: 'Удаление по фолам',
  fourth_foul_lost_label: 'Удаление по фолам на поражение',
  tech_foul_label: '{count}-й тех. фол',
  second_tech_foul_lost_label: 'Удаление по тех. фолам на поражение',
  suicide_label: 'Самоубийство',
  suicide_lost_label: 'Самоубийство на поражение',
  best_move_black_0_label: 'Лучший ход: 0 из 3',
  best_move_black_1_label: 'Лучший ход: 1 из 3',
  best_move_black_2_label: 'Лучший ход: 2 из 3',
...
function snapshotScoring(): string {
  return JSON.stringify({
    ...
    vote_black_unchecked_nine_ten: normalizeScoringValue(scoring.vote_black_unchecked_nine_ten),
    ...
    farewell_black_named_red: normalizeScoringValue(scoring.farewell_black_named_red),
    farewell_claimant_black_named_red: normalizeScoringValue(scoring.farewell_claimant_black_named_red),
    ...
    vote_black_unchecked_nine_ten_label: normalizeScoringLabel(scoring.vote_black_unchecked_nine_ten_label),
    ...
    farewell_black_named_red_label: normalizeScoringLabel(scoring.farewell_black_named_red_label),
    farewell_claimant_black_named_red_label: normalizeScoringLabel(scoring.farewell_claimant_black_named_red_label),
  })
}
...
async function loadScoring(): Promise<void> {
  ...
  try {
    ...
    scoring.vote_black_unchecked_nine_ten = normalizeScoringValue(scoring.vote_black_unchecked_nine_ten)
    ...
    scoring.farewell_black_named_red = normalizeScoringValue(scoring.farewell_black_named_red)
    scoring.farewell_claimant_black_named_red = normalizeScoringValue(scoring.farewell_claimant_black_named_red)
    ...
    scoring.vote_black_unchecked_nine_ten_label = normalizeScoringLabel(scoring.vote_black_unchecked_nine_ten_label)
    ...
    scoring.farewell_black_named_red_label = normalizeScoringLabel(scoring.farewell_black_named_red_label)
    scoring.farewell_claimant_black_named_red_label = normalizeScoringLabel(scoring.farewell_claimant_black_named_red_label)
...
async function updateSanctionReason(row: SanctionsRow, event: Event): Promise<void> {
  ...
  try {
    await api.patch(`/admin/sanctions/${row.id}/reason`, { reason: nextReason })
    ...
    if (status === 404 && detail === 'sanction_not_found') {
      void loadSanctions()
